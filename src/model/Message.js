  ...
  getViewElement(me = true) {// me -> mensagem é minha, por padrao
    ...
    switch (this.type) {
      ...
      case 'audio':
        ...
        audioEl.onended = e => {
          audioEl.currentTime = 0;//? quando terminar de tocar o audio, zera o currentTime
        }
        audioEl.ontimeupdate = e => {// quando o usuario, altera o range (linha do tempo) do audio
          btnPause.hide();
          btnPlay.hide();
        ...
        // atribuindo os eventos ao 'click'
        btnPlay.on('click', e => {
          audioEl.play();
        });
        btnPause.on('click', e => {
        ...
        inputRange.on('change', e => {
          audioEl.currentTime = (inputRange.value * this.duration) / 100;//* currentTime -> propriedade que pega o tempo do audio em que estamos
        });
        break;
      default:
        div.innerHTML = `
    ...
    // MESSAGE STATUS
    let className = 'message-in';// agora a mensagem por padrao nao eh minha
    if (me) {// e se for minha
      className = 'message-out';// atribui a classe
      /**
       * * agora pra add o elemento, primeiro, encontramos o elemento 'irmao'
       * * ai encontramos o pai -> parentElement
       * * e add um novo 'filho' -> appendChild()
       * * que na realidade, eh o nosso elemento criado -> metodo getStatusViewElement()
      */
      div.querySelector('.message-time').parentElement.appendChild(this.getStatusViewElement());// n precisa passar o status, pq ele automaticamente pega do this.status
    }
